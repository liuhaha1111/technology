...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    portalApi
      .getProfile()
      .then((res) => setProfile(res.data as PortalProfile))
    ...
    <aside className="w-72 bg-surface-light dark:bg-surface-dark border-r border-border-light dark:border-border-dark flex flex-col overflow-y-auto scrollbar-thin flex-none shadow-lg z-10">
      <div className="mb-2">
        ...
        <div className="p-4 space-y-3 text-xs text-muted-light dark:text-muted-dark border-b border-border-light dark:border-border-dark">
          <div className="flex">
            <span className="font-semibold w-24 flex-shrink-0">身份:</span>
            <span>{profile?.accountType === "principal" ? "项目负责人" : "-"}</span>
          </div>
          <div className="flex">
            <span className="font-semibold w-24 flex-shrink-0">所属科技管理部门:</span>
            <span>{profile?.organization?.department_name ?? "-"}</span>
          </div>
          <div className="flex">
            <span className="font-semibold w-24 flex-shrink-0">单位联系人:</span>
            <span>{profile?.principal?.full_name ?? "-"}</span>
          </div>
          <div className="flex">
            <span className="font-semibold w-24 flex-shrink-0">联系电话:</span>
            <span>{profile?.principal?.phone ?? profile?.organization?.contact_phone ?? "-"}</span>
          </div>
          <div className="flex">
            <span className="font-semibold w-24 flex-shrink-0">单位:</span>
