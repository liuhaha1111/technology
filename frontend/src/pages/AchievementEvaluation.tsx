...
export default function AchievementEvaluation() {
  const [activeTab, setActiveTab] = useState('提交成果评价');

  const tableData = [
    { id: 1, year: '', name: '具有长服务周期的工业复杂系统可适化方法研究', leader: '张昕', category: '' },
    { id: 2, year: '', name: '面向城市精细化治理的智能路径规划服务', leader: '张昕', category: '' },
    { id: 3, year: '', name: '基于位置服务发现的新能源交通设施规划方法研究', leader: '张昕', category: '' },
  ];
  ...
  return (
    <div className="p-4 bg-white dark:bg-surface-dark min-h-screen">
      <div className="mb-6 border-b-2 border-blue-200 dark:border-blue-800 pb-2 flex items-center">
        <span className="material-icons-outlined text-blue-500 mr-2">assignment</span>
        <h2 className="text-lg font-bold text-blue-500">成果评价</h2>
      </div>

      <div className="flex justify-end mb-4">
        <div className="flex bg-gray-100 dark:bg-gray-800 rounded-full p-1">
          {['填写成功评价', '提交成果评价'].map((tab) => (
            <button
              key={tab}
      ...
      <div className="overflow-x-auto border border-gray-200 dark:border-gray-700">
        <table className="w-full text-sm text-center">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {activeTab === '填写成功评价' && tableData.map((row) => (
              <tr key={row.id} className="hover:bg-blue-50 dark:hover:bg-gray-800 transition">
                ...
                <td className="py-3 px-4">
                  <span className="material-icons-outlined text-gray-400 hover:text-blue-600 cursor-pointer transition">description</span>
                </td>
              </tr>
            ))}
